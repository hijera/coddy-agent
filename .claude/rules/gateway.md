...
- `richmsg.go` (pure, table-tested): `buildRichMarkdown` (final message + one collapsed `<details>` block per executed tool with its output, via the `toolCall` type), `buildRichDraftMarkdown` (streaming preview + draft-only `<tg-thinking>` block while a tool runs), and `richFormattingHint` (replaces `telegramFormattingHint` on the first turn). The Sender captures tool args/results from `acp.ToolCallStatusUpdate`. `flushRich` retries with the answer alone if the combined message (answer + tool blocks) is rejected, so the reply is never lost.
