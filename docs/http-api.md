...
| GET | **`/coddy/skills`** | Lists all skills found in configured **`skills.dirs`** with their enabled/disabled state. JSON **`{"object":"coddy.skills_list","items":[...]}`**; each item has **`name`**, **`description`**, **`file_path`**, **`globs`**, **`disabled`**. Uses same directory scan and last-wins deduplication as the agent loader. |
