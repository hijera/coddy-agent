...
- **Composer chip:** an environment selector sits in the composer bar (**`EnvironmentChip.tsx`**, first chip in **`.composer-tabs`**, **`data-testid="composer-env-btn"`**), Claude-Code style — **not** in Settings. The chip shows **`Local`** or the remote's name with a status dot (amber when driving a remote). It opens a portal menu (**`data-testid="composer-env-menu"`**, mode-menu family; bottom sheet on mobile) with an **Environment** section (**Local**) and a **Remote** section (configured remotes + **`+ Add remote…`**).
...
- **Auth token:** the remote's **`auth_token`** is entered in the menu and stored **client-side only** — the active env in **`localStorage`** key **`coddy_env`**, plus a per-remote token map in **`coddy_env_tokens`** so re-selecting a known remote is one click. Never persisted to server config; leave empty for a remote without auth.
